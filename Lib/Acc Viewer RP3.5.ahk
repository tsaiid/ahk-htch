...
GetAccInfo() {
	...
	MouseGetPos, , , Whwnd
    SendMessage, WM_GETOBJECT := 0x003D, 0, 1, Chrome_RenderWidgetHostHWND1, % "ahk_id " Whwnd
...
UpdateAccInfo(Acc, ChildId, Obj_Path="") {
	...
	Location := GetAccLocation(Acc, ChildId, x,y,w,h)
	{
		...
		GuiControl, , AccRole, % Acc_GetRoleText(Stored.Role:= Acc.accRole(ChildId))
		GuiControl, , AccState, % Acc_GetStateText(Stored.State:= Acc.accState(ChildId))
    ...
    of:=5, DrawRectangle(1, 3, x-of, y-of, w+2*of, h+2*of)
...
DrawRectangle(n:=1, bw:=3, x1:=0, y1:=0, w1:=0, h1:=0, c1:="8800ff", c2:="ff000") { ; flyingDman, RRR
Static FirstCall1:= 1, FirstCall2:= 1
    if (w=0 && h=0) {
        Gui % n ": Destroy"
        Return
    }
    ...
    If (FirstCall%n%) {
        ...
        Gui % n ": Color", % c%n%
        Gui % n ": Show", NA x%x1% y%y1% w%w1% h%h1%
        FirstCall%n% := False
    }   Gui % n ": Show", NA x%x1% y%y1% w%w1% h%h1%
...
Anchor(i, a = "", r = false)
{
	...
	If (gp != gpi)
	{
		...
		If (!gf)
			NumPut(gp, g, gl, "UInt"), NumPut(gw := giw, g, gl + 4, "Short")
          , NumPut(gh := gih, g, gl + 6, "Short"), gl += gs
	}
...
#IfWinActive Accessible Info Viewer
$Esc:: Brk:= 1
f::
...
ParseAccData(accObj, accPath:="") {
    if  Stored.Location = (tLo:=GetAccLocation(accObj))
	&&  Stored.Name = AccObj.accName(0)
	&&  Stored.Value = AccObj.accValue(0)
	&&  Stored.Role = AccObj.accRole(0)
	;&&	Stored.State = AccObj.accState(0)
	&&	Stored.DefaultAction = AccObj.accDefaultAction(0)
	;;&&	Stored.ChildCount = accObj? "N/A": AccObj.accChildCount
	;;&&	Stored.Selection = accObj? "N/A": AccObj.accSelection
	;;&&	Stored.Focus = accObj? "N/A": AccObj.accFocus
	&&	Stored.Description = AccObj.accDescription(0)
	&&	Stored.ShortCut = AccObj.accKeyboardShortCut(0)
	&&	Stored.Help = AccObj.accHelp(0)
	&&	Stored.HelpTopic = AccObj.accHelpTopic(0)

        tPath.1:= Trim(accPath, "."), Brk:= 1
    sLc:= StrSplit(StrReplace(stored.Location, "  ", " "), " "), tLc:= StrSplit(StrReplace(tLo, "  ", " "), " ")
    For i, j in (sLc, sLoc:=[])
        sLoc.Push(SubStr(j, 2))
    For i, j in (tLc, tLoc:=[])
        tLoc.Push(SubStr(j, 2))
    if (sLoc.1>=tLoc.1 && sLoc.2>=tLoc.2 && (sloc.1+sLoc.3<=tLoc.1+tLoc.3) && (sloc.2+sLoc.4<=tLoc.2+tLoc.4) && !Brk)
        accPath? tPath.Push( Trim(accPath, ".") ): ""

    For nChild, accChild in (GetCurrentAccChildren(accObj), n:=0) {
        if Brk
            break
        currPath:= accPath nChild ".", ParseAccData(accChild, currPath)
    }   Return {AccObj: accObj, Path: tPath.1? tPath.1: StrReplace(txtArr(tPath), "1: ", "1: N/A",, 1)}
}

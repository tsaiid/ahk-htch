...
Right renal size about {1}; left renal size about {2}.
...
The visualized portion of spleen is normal. Spleen size about {1}.

Bilateral kidneys are normal in echogenicity.
Right kidney size about {2}; left kidney size about {3}.
No abnormal dilatation of bilateral urinary collecting systems noted.
)", Spleen, RightKidney, LeftKidney)
...
The uterus was measured as {1} x {2} x {3} cm in size.

The endometrium was measured about {4} cm in thickness.

The RT ovary was measured as {5} x {6} x {7} cm ({8} ml) in size.

The LT ovary was measured as {9} x {10} x {11} cm ({12} ml) in size.


Impression: Essentially normal study.
)", UterusL, UterusH, UterusW, Endometrium, RightOvaryL, RightOvaryH, RightOvaryW, RightOvaryVol, LeftOvaryL, LeftOvaryH, LeftOvaryW, LeftOvaryVol)
    Paste(MyForm)
}
...
The prostate is measured about {1} in volume.
...
::sprok::{
    ...
    MyForm := Format("
(
No definite focal lesion in the peripheral zone of prostate, normal appearance of the inner glands, with the prostate measured about {1} in volume.
Normal appearance of the seminal vesicles.
)", ProstateVol)
    Paste(MyForm)
}
